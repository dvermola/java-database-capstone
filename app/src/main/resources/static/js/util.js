 ...
 * @param role - The role of the user. Examples: "admin", "patient", "doctor".
 * @returns void No return value. This function simply stores the role in the `localStorage` under the key `"userRole"`.
 */
...
/**
 * **Purpose**: Clears the user's role from `localStorage`. This is useful for logging out users and removing any session data.<br>
 * **Use Case**: This function is typically called during the logout process to clear all session-related data.
 * @returns void No return value. This function simply removes the role from `localStorage` using the key `"userRole"`.
 */
function clearRole() {
...
/**
 * **Purpose**: Retrieves the current user's role from `localStorage`.<br>
 ...
 * @returns {string} A string representing the user's role (e.g., "admin", "patient", "doctor") or `null` if no role is set.
 */
function getRole() {
